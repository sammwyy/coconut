...
/// The device panel wrapper opened directly by the `"tray.bluetooth"` island
...
/// - [`crate::BluetoothRevision`] (`Signal<()>`) — read for its reactive
///   subscription only.
///
/// This panel's own `scratch` additionally holds (lazily created, no
/// pre-population needed): [`crate::BluetoothScroll`],
/// [`crate::BluetoothDetailView`].
pub struct BluetoothPanel;

impl Panel for BluetoothPanel {
...
/// The bluetooth panel's actual content, shared between [`BluetoothPanel`]
/// and [`crate::panels::control_center::ControlCenterPanel`]'s embedded
/// Bluetooth view. Ported from
/// `apps/shell/src/panels/bluetooth/mod.rs::build`.
pub fn build_content(
    shared: &SharedState,
    scratch: &SharedState,
    on_back: Option<Rc<dyn Fn()>>,
) -> BoxedWidget {
    let bluetooth = shared
        .get::<Rc<dyn BluetoothIntegration>>()
        .unwrap_or_else(|| Rc::new(BluetoothFallback));
    let bluetooth_powered = shared
        .get::<BluetoothPowered>()
        .unwrap_or_else(|| BluetoothPowered(Signal::new(false)))
        .0;
    if let Some(revision) = shared.get::<BluetoothRevision>() {
        revision.0.get();
    }
    let scroll = scratch
        .get_or_insert_with(|| BluetoothScroll(ScrollController::new(0.0)))
        .0;
    let view = scratch
        .get_or_insert_with(|| BluetoothDetailView(Signal::new(None)))
        .0;
    ...
    if !powered {
        ...
        // a scan in progress has merely discovered nearby, each under its
        // own label.
