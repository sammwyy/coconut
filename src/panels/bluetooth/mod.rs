...
use crate::integrations::bluetooth::{BluetoothDevice, BluetoothIntegration};
use crate::panels::chrome::{
    action_button, compact_switch, detail_row, icon_button, list_row, panel_header,
    section_label, BORDER, CARD, CARD_RADIUS, ISLAND_RADIUS, LIST_WIDTH, MUTED, PANEL,
};
...
pub fn build(
    ...
    let devices = if powered { bluetooth.devices() } else { Vec::new() };
...
fn discovered_device_row(device: &BluetoothDevice, bluetooth: Rc<dyn BluetoothIntegration>) -> BoxedWidget {
    let icon = device_row_icon(device);
    let address = device.address.clone();
...
fn class_device_icon(class: Option<u32>) -> Option<&'static str> {
    ...
    (major == MAJOR_AUDIO_VIDEO
        && matches!(minor, MINOR_SET_TOP_BOX | MINOR_VIDEO_DISPLAY_AND_LOUDSPEAKER))
        .then_some("tv")
}
