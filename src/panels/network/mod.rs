...
use crate::integrations::network::{NetworkDevice, NetworkDeviceKind, NetworkIntegration, WifiNetwork};
...
fn connection_card_for_kind(
    ...
    match devices.iter().find(|device| device.kind == kind && device.active) {
        Some(device) => {
            let title = device
                .connection_name
                .clone()
...
fn password_row(ssid: &str, network: Rc<dyn NetworkIntegration>, password: Signal<Option<String>>) -> BoxedWidget {
...
fn wifi_section_header(
    ...
    let action: BoxedWidget = if !wifi_on {
        ...
        Box::new(RawSpinner::new(MUTED).size(14.0))
    } else {
        icon_button("scan", 22.0, on_rescan)
    };
    Box::new(jsx! {
        <Flex direction={FlexDirection::Row} align={Align::Center} gap={8.0} padding={4.0}>
            <RawText color={MUTED} font_size={11.0} align={TextAlign::Start}>"WI-FI NETWORKS"</RawText>
