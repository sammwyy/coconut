...
pub fn connection_card(
    ...
    match on_click {
        Some(handler) => {
            Box::new(RawButton::new(Style::new().corner_radius(ISLAND_RADIUS), move || handler()).child(card))
        }
        None => card,
    }
}
...
pub fn list_row(
    ...
    match on_click {
        Some(handler) => Box::new(RawButton::new(list_row_style(active), move || handler()).child(content)),
        None => Box::new(jsx! {
